...
! PARALLELOGRAM 
REAL(F64) FUNCTION POT_PAR1(A,P,H,X,Y,Z)
    REAL(F64)::A,P,H,X,Y,Z,Z2
    ...
    UR=BR/(A1L*A2L)
    UL=BL/(A1L*A2L)
    POT_PAR1=(FPSI(SR+H,UR,VL,W2R)-FPSI(SR,UR,VL,W2R)+FPSI(SL,UL,VL,W2L)-FPSI(SL+H,UL,VL,W2L))*A2L;
ENDFUNCTION POT_PAR1
 
! RECTANGULAR, P=0 
REAL(F64) FUNCTION POT_RECT1(A,H,X,Y,Z)
    REAL(F64)::A,H,X,Y,Z
    REAL(F64)::Z2,SR,SL,W2L,W2R,UL,UR,BL,BR
    Z2=Z**2
    IF(Z2<ZZERO) Z2=ZZERO
    BL=-X;BR=-X+A
    SR=-Y
    ...
    UR=BR
    UL=BL
    POT_RECT1= FPSI0(SR+H,UR,W2R)-FPSI0(SR,UR,W2R)+FPSI0(SL,UL,W2L)-FPSI0(SL+H,UL,W2L);
ENDFUNCTION POT_RECT1
