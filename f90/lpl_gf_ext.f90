...
FUNCTION DLPL_GF_L1(DST,VRT0,VRT1,VRT2,Q0,Q1,Q2) RESULT (RES)
    USE DATA_BUF
    REAL(F64)::DST(3),VRT0(3),VRT1(3),VRT2(3)
...
FUNCTION FF(WTT,WBT, WTB, WBB,QT, QB,VT, VB,ZZ, A, B,C) RESULT(RES)
    ...
    REAL(F64) :: RES,res1,res2
    res1=(-NEWG(WTT,VT,QT,ZZ,A,B,C)+NEWG(WBT,VT,QT,ZZ,A,B,C))/(VT*VT+ONE_F64)
    res2=(NEWG(WTB,VB,QB,ZZ,A,B,C)-NEWG(WBB,VB,QB,ZZ,A,B,C))/(VB*VB+ONE_F64)
    RES= res1+res2
END FUNCTION FF
