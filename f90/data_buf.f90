...
CONTAINS
#ifndef NONEWUNIT
#define OPEN_R(FNAME) OPEN(NEWUNIT=LUN,FILE=FNAME,ACCESS="STREAM",IOSTAT=FLAG,STATUS="OLD")
#define OPEN_W(FNAME) OPEN(NEWUNIT=LUN,FILE=FNAME,ACCESS="STREAM",IOSTAT=FLAG,STATUS="REPLACE")
#else
#define OPEN_R(FNAME) OPEN(UNIT=GET_LUN(LUN),FILE=FNAME,ACCESS="STREAM",IOSTAT=FLAG,STATUS="OLD")
#define OPEN_W(FNAME) OPEN(UNIT=GET_LUN(LUN),FILE=FNAME,ACCESS="STREAM",IOSTAT=FLAG,STATUS="REPLACE")
#endif
...
SUBROUTINE READ_ARR_7_F32(FNAME,FLAG,A,N1,N2,N3,N4,N5,N6,N7)
    ...
    INTEGER::LUN,H1(HDR_SIZE),H2(HDR_SIZE)
    OPEN_R(FNAME)
    IF(FLAG/=0) RETURN
    CALL MK_HDR_7_F32(H1,N1,N2,N3,N4,N5,N6,N7)

    READ(LUN,IOSTAT=FLAG) H2
    IF(FLAG/=0) THEN
        CLOSE(LUN)
        RETURN
    ENDIF
    IF(CMP_HDR(H1,H2)) THEN
        READ(LUN,IOSTAT=FLAG) A
        CLOSE(LUN)
        RETURN
    ELSE
        FLAG=-1
...
SUBROUTINE WRITE_ARR_7_F32(FNAME,FLAG,A,N1,N2,N3,N4,N5,N6,N7)
    ...
    REAL(F32)::A(N1,N2,N3,N4,N5,N6,N7)
    INTEGER::N1,N2,N3,N4,N5,N6,N7
    INTEGER::LUN,H1(HDR_SIZE)
    OPEN_W(FNAME)
    IF(FLAG/=0) RETURN
    CALL MK_HDR_7_F32(H1,N1,N2,N3,N4,N5,N6,N7)
    WRITE(LUN,IOSTAT=FLAG) H1,A
    CLOSE(LUN)
...
SUBROUTINE WRITE_CDATA_F32(D,FNAME,FLAG)
    ...
    OPEN_W(FNAME)
    IF(FLAG>0) RETURN
    WRITE(LUN,IOSTAT=FLAG) D%HDR,D%BUF
    CLOSE(LUN)
