...
static inline double newg(double u,double v,double q,double zz,
	...
	//printf("u=%f v=%f q=%f zz=%f a=%f b=%f c=%f\n",u,v,q,zz,a,b,c);
	vv=v*v;
	vv2=(vv+1.0);
	vv1=vv2-2.0;
	b/=vv2;
	...
	g6=(L1-L2)*bvvqq;
	//g3=L2*(a*v*ww+b*ww-bvvqq+b*s2t2-bqv*vv1*ts+2.0*q*c+cvts);
	//g4=L3*(b*s2t2-bqv*vv1*ts+cvts);
	//g5=L1*(bqv*vv1*ts-b*s2t2+bvvqq-0.5*(vv*vv+1.0)*ww*b+c*vv1*q-cvts);
...
double trgint(double x,double y,double z,
	...
	if(d1<EPS) return 0.0;//triangle is degenerate
    
	x20=x2-x0;
	y20=y2-y0;
	z20=z2-z0;
	d2=sqrt(x20*x20+y20*y20+z20*z20);
	if(d2<EPS) return 0.0;//triangle is degenerate
    ...
    return res;
	//return ONEOVER4PI*res;

}
static inline long double newg_l(long double u,long double v,long double q,long double zz,
		long double a,long double b,long double c) {
	long double vv,z,vz,svq,s,t,ww,qq,vq,ss,tt,bqv;
	long double g1,g2,g3,g4,g5,L1,L2,L3,g6;
	long double vv2,vv1,bvvqq,ts,s2t2,cvts;
	//printf("u=%f v=%f q=%f zz=%f a=%f b=%f c=%f\n",u,v,q,zz,a,b,c);
	vv=v*v;
	vv2=(vv+1.0);
	vv1=vv2-2.0;
...
static inline long double ff_l(long double wtt,long double wbt,long double wtb,long double wbb,
		long double qt,long double qb,long double Vt,long double Vb,
		long double zz,long double a,long double b,long double c) {
	long double res;
	res=(-newg_l(wtt,Vt,qt,zz,a,b,c)+newg_l(wbt,Vt,qt,zz,a,b,c))/(Vt*Vt+1.0)
		+(newg_l(wtb,Vb,qb,zz,a,b,c)-newg_l(wbb,Vb,qb,zz,a,b,c))/(Vb*Vb+1.0);
	return res;
}

double trgint_l(double x,double y,double z,
             double x0,double y0,double z0,double q0,
             double x1,double y1,double z1,double q1,
             double x2,double y2,double z2,double q2) {
	
	long double x10,y10,z10,x20,y20,z20;
	long double nx,ny,nz,kx,ky,kz,n2x,n2y,n2z;
	long double d1,d2,c2,s2,u2,u1,v2,zz,u3,v3,v4,a,b,c;
	long double Pt,pt,pb,qt,qb,Pb,ppt,ppb,wtt,wbb,wtb,wbt,Vb,Vt,res/*,norm=0.25/M_PI*/;
	...
	res=ff_l(wtt,wbt,wtb,wbb,qt,qb,Vt,Vb,zz,a,b,c);
    return res;
	//return ONEOVER4PI*res;
